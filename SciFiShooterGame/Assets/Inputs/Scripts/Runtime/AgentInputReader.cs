...
{
    /// <summary>
    /// Per-agent input state. Replaces the InputReader ScriptableObject, whose fields were shared by
    /// every Agent in the scene: on a host, the remote player's Agent read the host's own input.
    /// Only the owning client ever calls <see cref="InitializeControls"/>; on remote agents every
    /// value stays at its default, so their handlers read a neutral input.
    /// </summary>
    public class AgentInputReader : MonoBehaviour, Controls.ICharacterActions, Controls.IWeaponActions
    {
        ...
        private void OnDestroy() => DestroyControls();
